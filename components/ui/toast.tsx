...
const ToastAction = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Action>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Action>>(
  ({ className, ...props }, ref) => (
    <ToastPrimitives.Action
      ref={ref}
      className={cn(
        'inline-flex h-9 shrink-0 items-center justify-center rounded-lg border-2 bg-transparent px-4 text-sm font-semibold shadow-sm',
        'transition-all duration-200',
        'hover:bg-default-100 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:ring-offset-2',
        'disabled:pointer-events-none disabled:opacity-50',
        'group-[.destructive]:border-white/40 group-[.destructive]:hover:border-white group-[.destructive]:hover:bg-white/10 group-[.destructive]:focus:ring-white/20',
        className
      )}
      {...props}
    />
...
const ToastClose = React.forwardRef<React.ElementRef<typeof ToastPrimitives.Close>, React.ComponentPropsWithoutRef<typeof ToastPrimitives.Close>>(
  ({ className, ...props }, ref) => (
    <ToastPrimitives.Close
      ref={ref}
      className={cn(
        'absolute right-3 top-3 rounded-lg p-1.5 text-default-500 opacity-70 transition-all duration-150',
        'hover:opacity-100 hover:bg-default-100 focus:opacity-100 focus:outline-none focus:ring-2 focus:ring-primary/20',
        'group-hover:opacity-100',
        'group-[.destructive]:text-white/70 group-[.destructive]:hover:text-white group-[.destructive]:hover:bg-white/10 group-[.destructive]:focus:ring-white/20',
        className
      )}
      toast-close=""
      {...props}
